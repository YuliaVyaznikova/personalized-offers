...
def load_and_prepare_data(input_json_path):
    ...
    for product_id, revenue in product_revenues.items():
        csv_name = f"df_{iii}_filtered.csv"
        iii+=1
        if iii == 3:
            iii+=1
        if os.path.exists(csv_name):
            ...
            try:
                ...
                df = df[['user_id', 'lama_proba']]
                # Нормализуем имена колонок для надежности
                df.columns = ['user_id', 'affinity_prob']
    ...
    df_affinity = pd.concat(affinity_dfs, ignore_index=True)
...
def generate_reports(df_results, budget, product_revenues, channels_data, channel_limits, output_json_path,
    ...
    out_columns = ['product_id', 'user_id', 'canal_id']
...
def create_dummy_environment():
    """
    Создает input.json и CSV файлы для проверки работы скрипта.
    """
    ...
    config = {
        "model": "test_model",
        "budget": 5000,  # Небольшой бюджет, чтобы алгоритм выбирал
        "advanced": True,
        "enable_rr": True,
        "channels": {
            "sms": [50, 2.0, 0.05],  # [limit, cost, prob]
            "email": [100, 0.5, 0.02],
            "push": [100, 1.0, 0.03],
            "phone": [5, 50.0, 0.30]  # Очень дорогой и лимитированный канал
        },
        "products": {
            "credit_card": 3000,
            "debit_card": 500,
    ...
    # 2. Создаем CSV файлы для продуктов (20 клиентов)
    n_clients = 20
    client_ids = [f'client_{i}' for i in range(n_clients)]

    for prod in config['products']:
        # Генерируем вероятности (смещенные к 0)
        probs = np.random.beta(2, 5, n_clients)
        ...
        df.to_csv(f'{prod}.csv', index=False)

    print("Тестовые файлы созданы: input.json и *.csv")
